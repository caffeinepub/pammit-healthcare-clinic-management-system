...
export default function StaffReportPage() {
  ...
  const positionCounts = staff.reduce<Record<string, number>>((acc, member) => {
  ...
  const mostCommonRole = sortedPositions.length > 0 ? sortedPositions[0][0] : 'N/A';
    ...
    <div className="container py-8 px-4 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-lg font-bold">{mostCommonRole}</div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="space-y-2">
            {sortedPositions.map(([position, count]) => (
              <div key={position} className="flex items-center justify-between border-b pb-2 last:border-0">
                ...
                <span className="text-muted-foreground">{count as number} member{(count as number) !== 1 ? 's' : ''}</span>
              </div>
            ))}
